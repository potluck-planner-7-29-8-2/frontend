...
const Guests = (props) => {
  ...
  const userCompare = (toCompare) => {
    ...
    searchGuests = searchGuests.filter(user =>  user.full_name.toLowerCase() === toCompare.toLowerCase())
    ...
    return (
        ...
        <ul>{displayedUsers.map(user => {
            return <li key={user.user_id}>{user.full_name}<button onClick={(e) => {
                e.preventDefault();
                addGuest(dispatch, props.eventID, {'user_id' : user.user_id, 'attending' : true})
            }}>Invite</button></li>
        })}</ul>
        </div>
    )
